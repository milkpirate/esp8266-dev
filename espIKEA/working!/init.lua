...
print('AP ssid/pass: ',wifiConfig.accessPointConfig.ssid.." / "..wifiConfig.accessPointConfig.pwd)
print('Sta ssid/pass: ',wifiConfig.stationPointConfig.ssid.." / "..wifiConfig.stationPointConfig.pwd)
...
tmr.alarm(0, 1000, 1, function()
    ...
    if ip == nil and jcnt <= jmax then
        ...
        if jcnt > jmax then
            ...
            wifi.sta.setip({ip=ip:gmatch('%d+.%d+.%d+.')()..nip})
            print('Sta: ',wifi.sta.getip())
            print('AP:  ',wifi.ap.getip())
            if file.open(fname,"r") then
                file.close()
                print("Start "..fname.." in 1s...")
                tmr.alarm(0, 1000, 0, function() dofile(fname) end)
            else
                print(fname.." not found.")
            end
        end
        nip, ip, jcnt, jmax = nil, nil, nil, nil
    end
end)
